...
from casadi import SX, MX, nlpsol, integrator, vertcat
from casadi import sin, cos, exp, pi, inf
...
quad = u
# dae = Function("dae", daeIn(x=x, p=vertcat([u, T])), daeOut(ode=T*vertcat(ode), quad=T*quad))
dae = {'x':x, 'p':vertcat(u, T), 'ode':T*vertcat(*ode), 'quad':T*quad}


I = integrator("I", "cvodes", dae, 0.0, 1.0 / N)
...
nlp_prob = {'x':V,'f':m0 - X[-1][0], 'g':vertcat(*G)}
opts = {'ipopt.tol': 1e-5}
...
f = float(sol['f'])
T = w_opt[0]
t = [float(i * (T / N)) for i in range(0, N+1)]
